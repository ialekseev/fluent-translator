  ...
  "Requesting token for the first time" when {
    ...
    "remote service throws exception" should {
      "fail with that exception" in {
        ...
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Failure(new RuntimeException("Can't connect")))
    ...
    "remote service returns http error" should {
      "fail with an exception" in {
        //arrange
        val actor = system.actorOf(Props(new TokenProviderActor("my-client-id", "my-client-secret", httpClient)))
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Success(ErrorHttpResponse("Error!")))
    ...
    "remote service returns success but json is invalid" should {
      "fail with ParseException" in {
        //arrange
        val actor = system.actorOf(Props(new TokenProviderActor("my-client-id", "my-client-secret", httpClient)))
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Success(SuccessHttpResponse("Bad json")))
    ...
    "remote service returns success and json is good" should {
      "get token" in {
        ...
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Success(SuccessHttpResponse("""{"access_token": "123abc", "expires_in": "1001"}""")))
  ...
  "Requesting token" when {
    "current token is still valid" should {
      "just return it" in {
        ...
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Success(SuccessHttpResponse("""{"access_token": "123abc", "expires_in": "600"}""")))
        actor ! TokenRequestMessage
        ...
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Success(SuccessHttpResponse("""{"access_token": "123abc_new", "expires_in": "1000"}""")))
    ...
    "current token has expired" should {
      "get a new token" in {
        ...
        currentTime =  2000 + 600 * 1000 - tokenExpirationDeltaInMillis + 1
        when(httpClient.post[String](HttpClientBasicRequest(requestAccessTokenUri), params)).thenReturn(Success(SuccessHttpResponse("""{"access_token": "123abc_new", "expires_in": "1500"}""")))
